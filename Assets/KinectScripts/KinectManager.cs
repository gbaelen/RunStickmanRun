	...
	public void DetectGesture(uint UserId, KinectGestures.Gestures gesture)
	{
		...
		switch(gesture)
		{
            ...
            case KinectGestures.Gestures.SwipeRight:
                gestureData.checkForGestures.Add(KinectGestures.Gestures.SwipeRight);
                break;
					...
					else if(gestureData.progress >= 0.1f)
					{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								...
								if(ControlMouseCursor)
								{
                                    Vector3 vCursorPos = HandCursor1.transform.position;
						...
						for(int g = 0; g < listGestureSize; g++)
						{
								...
								!IsConflictingGestureInProgress(gestureData))
							{
                                ...
                                //if(gestureData.state > 0)
                                {
                                    ...
                                    if (gestureData.gesture == KinectGestures.Gestures.SwipeRight)
                                    {
                                       if (gestureData.progress >= 0.1)
                                       {
                                           player.GetComponent<Running>().verifyjump(); 
                                           DeleteGesture(Player1ID, KinectGestures.Gestures.SwipeRight);
                                           gestureData.progress = 0.0f;
                                           gestureData.state = 0;
                                           ClearKinectUsers();
                                       }
                                    }
